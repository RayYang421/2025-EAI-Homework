...
class Bottleneck(nn.Module):
    ...
    def __init__(self, in_channels, planes, out_channels, downsample=None, stride=1):
        ...
        self.conv3 = nn.Conv2d(out_channels[1], planes * 4, kernel_size=1, stride=1, padding=0, bias=False)
...
class ResNet(nn.Module):
    ...
    def _make_layer(self, block, planes, blocks, cfg, stride=1):
        layers = []
        slice_len = 3 if block.expansion == 4 else 2
        # 每個 block 的輸出通道
        block_out_channels = planes * block.expansion
        for i in range(blocks):
            # stride 只在第一個 block 起作用，後面都用 1
            blk_stride = stride if i == 0 else 1

            downsample = None
            if i == 0 and (blk_stride != 1 or self.inplanes != block_out_channels):
                downsample = nn.Sequential(
                    nn.Conv2d(self.inplanes, block_out_channels, kernel_size=1, stride=blk_stride, bias=False),
                )

            layers.append(
                block(self.inplanes, planes, cfg[self.current_cfg_idx:self.current_cfg_idx + slice_len],
                      downsample, blk_stride)
            )

            # 更新 inplanes 與 cfg index
            self.inplanes = block_out_channels
            self.current_cfg_idx += slice_len
